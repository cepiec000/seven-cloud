...
import cc.seven.gateway.enable.entity.*;
import cc.seven.gateway.enable.service.*;
...
public class RouteEnhanceServiceImpl implements RouteEnhanceService {
    private static final Logger log=LoggerFactory.getLogger(RouteEnhanceServiceImpl.class);
    ...
    public Mono<Void> filterBalckList(ServerWebExchange exchange) {
        ...
        try {
            ...
            if (originUri != null) {
                String requestIp = SevenUtil.getServerHttpRequestIpAddress(request.getHeaders(),request.getRemoteAddress());
                ...
                if (forbid.get()) {
                    return SevenUtil.makeWebFluxResponse(response, MediaType.APPLICATION_JSON_VALUE,
                            HttpStatus.NOT_ACCEPTABLE, ApiResult.fail("黑名单限制，禁止访问"));
                }
            } else {
    ...
    public void saveRequestLogs(ServerWebExchange exchange) {
        ...
        if (!StringUtils.equalsIgnoreCase(TOKEN_CHECK_URL, originUri.getPath())) {
            ...
            ServerHttpRequest request = exchange.getRequest();
            String ipAddress = SevenUtil.getServerHttpRequestIpAddress(request.getHeaders(),request.getRemoteAddress());
    ...
    private Mono<Void> doRateLimitCheck(AtomicBoolean limit, RateLimitRule rule, URI uri,
        ...
        if (limit.get()) {
            ...
            if (count == 0) {
                ...
                return SevenUtil.makeWebFluxResponse(response, MediaType.APPLICATION_JSON_VALUE,
                        HttpStatus.TOO_MANY_REQUESTS, ApiResult.fail("访问频率超限，请稍后再试"));
            } else {
