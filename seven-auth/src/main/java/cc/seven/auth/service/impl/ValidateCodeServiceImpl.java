...
import cc.seven.auth.properties.SevenValidateCodeProperties;
import cc.seven.auth.service.ValidateCodeService;
import cc.seven.common.core.constant.ImageTypeConstant;
...
public class ValidateCodeServiceImpl implements ValidateCodeService {
    ...
    public void create(HttpServletRequest request, HttpServletResponse response) throws IOException, ValidateCodeException {
        ...
        SevenValidateCodeProperties code = properties.getCode();
        setHeader(response, code.getType());

        Captcha captcha = createCaptcha(code);
        redisService.set(SevenConstant.CODE_PREFIX + key, StringUtils.lowerCase(captcha.text()), code.getTime());
        captcha.out(response.getOutputStream());
    }

    @Override
    public void check(String key, String value) throws ValidateCodeException {
        Object codeInRedis = redisService.get(SevenConstant.CODE_PREFIX + key);
    ...
    private Captcha createCaptcha(SevenValidateCodeProperties code) {
